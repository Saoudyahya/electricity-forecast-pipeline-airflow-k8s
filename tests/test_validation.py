...
def main():
    ...
    try:
        ...
        if 'outliers' in stats:
            ...
            logger.info(f"   Count: {outliers['count']}")
            logger.info(f"   Percentage: {outliers['percentage']:.2f}%")
            logger.info(f"   Lower bound: {outliers['bounds']['lower']:.2f}")
            logger.info(f"   Upper bound: {outliers['bounds']['upper']:.2f}")
